    ...
    <footer class="bg-blue-950" aria-labelledby="footer-heading">
      ...
      <div class="mx-auto max-w-7xl px-6 pb-8 pt-16 sm:pt-24 lg:px-8 lg:pt-12">
        <div class="xl:grid xl:grid-cols-2 xl:gap-8 ">
          <div class="space-y-4">
            <img class="h-7" src="/images/Backslash-logo.png" alt="Company name" />
            <p class="text-sm leading-6 text-white">Redefining how people connect with cutting-edge solutions.</p>
          ...
          <div class="mt-10  xl:mt-0 ">
            <div class="flex justify-around ">
              <div>
                ...
                <ul role="list" class="mt-6 space-y-4">
                  <li v-for="item in navigation.services" :key="item.name">
                    <a :href="item.href" class="text-sm leading-6 text-white hover:text-white">{{ item.name }}</a>
                  </li>
                </ul>
              </div>
              <div class="mt-10 md:mt-0">
                ...
                <ul role="list" class="mt-6 space-y-4">
                  <li v-for="item in navigation.about_us" :key="item.name">
                    <a :href="item.href" class="text-sm leading-6 text-white hover:text-white">{{ item.name }}</a>
                  </li>
                </ul>
              </div>
              <div>
                ...
                <ul role="list" class="mt-6 space-y-4">
                  <li v-for="item in navigation.contact_us" :key="item.name">
                    <a :href="item.href" class="text-sm leading-6 text-white hover:text-white">{{ item.name }}</a>
                  </li>
                </ul>
              </div>
        ...
        <div class="">
            <p class="mt-10 text-center text-xs leading-5 text-white">&copy; 2023 Backslash. All rights reserved.</p>
        </div>
      </div>
    </footer>
  </template>
    ...
    social: [
      {
        ...
        icon: defineComponent({
          render: () =>
            h('svg', { fill: '#ffffff', viewBox: '0 0 24 24' }, [
              h('path', {
                'fill-rule': 'evenodd',
